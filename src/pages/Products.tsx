...
const Products = () => {
  ...
  const categories = [
    ...
    { name: "Drinking Equipments", icon: Droplets, count: products.filter(p => p.category === "Drinking Equipments").length },
...
const CategorySidebarButton = ({ category, isActive, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`ml-auto text-xs px-2 py-0.5 rounded ${isActive ? "bg-red-200" : "bg-gray-200"}`}>
        {category.count}
      </span>
    </button>
  );
...
// Product Card (id/specifications fixed for backend data)
